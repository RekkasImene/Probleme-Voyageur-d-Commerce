...
{
     if (e->ns==n){
    return poids[e->chem[e->ns-1]][0];
   }
   int h = ((double)minLigne(e->chem[e->ns-1])/2);
   int i;

   for(i=0;i<n;i++)
   {
       if (!Existe_dans_chemin(i,e))
       {
           h=h+(double)(minLigne(i)/2)+(double)(minColumn(i)/2);
       }
   }
   h=h+(double)(minColumn(e->chem[0])/2);
...
void init_graph( int nbsommet )
{
   ...
   for (i=0; i<n; i++) {
	for (j=i+1; j<n; j++) {
	     poids[i][j] = 1+(int) (100.0*rand()/(RAND_MAX+1.0));	// entre 1 et 100
	     poids[j][i] = poids[i][j];
	}
	poids[i][i] = 0;
   }
...
void branch_bound( int depart )
{
   ...
   while ( !FileVide(f) && !stop ) {
	...
	if ( ch->ns == n ) {	// s'il contient tous les sommets, alors c'est la solution optimale
	   // MAJ cycle optimal
	   ...
	   for (j=0; j<n; j++)		// générer les succ et enfiler les nouveaux chemins ...
...
// Affichage de la matrice des coûts et sauvegarde dans le fichier texte "graph_bb_estim_heap.txt"
void affich()
{
   int i, j;
   FILE *f = fopen("graph_bb.txt", "w");
   fprintf(f, "%d\n", n);
